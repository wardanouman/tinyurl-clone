...
export default function Home({ onAddLink }) {
  ...
  const handleCopy = () => {
    ...
    const urlToCopy = recentLink.shortUrl || `${API_BASE_URL}/${recentLink.shortId}`;
    navigator.clipboard.writeText(urlToCopy);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };

  const shortUrl = recentLink ? (recentLink.shortUrl || `${API_BASE_URL}/${recentLink.shortId}`) : '';

  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <section className="max-w-4xl mx-auto pt-16 pb-12 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold mb-6 border border-blue-200">
            <Sparkles className="w-3.5 h-3.5" /> Next-Gen Link Management Platform
          </div>
          ...
          <p className="text-slate-600 text-lg max-w-2xl mx-auto mb-10">
            A powerful URL shortener designed to build brand recognition, improve click-through rates, and deliver real-time data insights.
          </p>
        ...
        <section className="bg-white py-16 border-y border-slate-200 -mx-4 sm:-mx-6 lg:-mx-8">

  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(data.error || data.msg || 'Failed to shorten URL');
      }
    } catch (err) {
    ...
    <div className="relative min-h-[calc(100vh-4rem)] bg-slate-50 text-slate-800 overflow-hidden">
      ...
      <section className="relative max-w-4xl mx-auto pt-16 pb-12 px-6 text-center">
        ...
        <div className="bg-white/90 backdrop-blur-xl p-8 md:p-10 rounded-3xl shadow-2xl shadow-blue-500/10 border border-slate-200/80 text-left transition-all hover:border-slate-300">
          ...
          {recentLink && (
            <div className="mt-8 pt-6 border-t border-slate-100 bg-gradient-to-r from-blue-50/60 to-indigo-50/40 p-5 rounded-2xl border border-blue-100/80">
              <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="truncate w-full">
                  <p className="text-xs font-extrabold text-blue-700 uppercase tracking-wider mb-1">
                    Your Shortened Link:
                  </p>
                  <a href={shortUrl} target="_blank" rel="noreferrer" className="text-xl font-bold text-blue-600 hover:underline truncate block">
                    {shortUrl}
                  </a>
                </div>

                <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
                  <button onClick={handleCopy} className="flex-1 md:flex-none px-5 py-2.5 bg-white text-blue-600 font-bold text-sm rounded-xl border border-blue-200 hover:bg-blue-50 transition shadow-sm flex items-center justify-center gap-2">
                    {copied ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4" />}
                    {copied ? 'Copied!' : 'Copy'}
                  </button>

                  <button onClick={() => setShowQR(!showQR)} className="flex-1 md:flex-none px-5 py-2.5 bg-blue-600 text-white font-bold text-sm rounded-xl hover:bg-blue-700 transition shadow-md shadow-blue-500/20 flex items-center justify-center gap-2">
